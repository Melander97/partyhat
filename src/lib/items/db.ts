...
/**
 * The shared item-pool filter: members-only (better signal-to-noise than
 * f2p commodity items), in the game pool, has both high and low prices set
 * (tradeable and recently active), and at least 100gp average (excludes
 * near-worthless items). getStartingPair and getRandomChallenger both
 * select against this same criteria.
 */
 ...
 * filters as getStartingPair, excluding ids already seen this run so a run
